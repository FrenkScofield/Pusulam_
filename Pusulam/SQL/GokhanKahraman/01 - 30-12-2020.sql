...
--################### DIS OGRENCI TABLOSU OLUSTURMA ############################

USE [Pusulam]
GO

/****** Object:  Table [dbo].[DisOgrenci]    Script Date: 24.12.2020 17:00:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE TABLE [dbo].[DisOgrenci](
	...
	[AKTIF] [bit] NULL,
 CONSTRAINT [PK_DisOgrenci] PRIMARY KEY CLUSTERED 
(
	[ID_DIS_OGRENCI] ASC
...
ALTER TABLE [dbo].[DisOgrenci] ADD  CONSTRAINT [DF_Table_1_GRUPID]  DEFAULT ((0)) FOR [ID_KADEME3]
GO

ALTER TABLE [dbo].[DisOgrenci] ADD  CONSTRAINT [DF_Table_1_ID_DONEM]  DEFAULT ((0)) FOR [DONEM]
GO

ALTER TABLE [dbo].[DisOgrenci] ADD  CONSTRAINT [DF_DisOgrenci_SIFRE]  DEFAULT ('okyanus') FOR [SIFRE]
GO

ALTER TABLE [dbo].[DisOgrenci] ADD  CONSTRAINT [DF_DisOgrenci_AKTIF]  DEFAULT ((1)) FOR [AKTIF]
GO


--################### DIS OGRENCI SP OLUSTURMA ############################

USE [Pusulam]
GO

/****** Object:  StoredProcedure [dbo].[sp_DisOgrenci]    Script Date: 30.12.2020 12:08:34 ******/
USE [Pusulam]
GO

/****** Object:  StoredProcedure [dbo].[sp_DisOgrenci]    Script Date: 30.12.2020 13:46:26 ******/
SET ANSI_NULLS ON
GO
...
SET NOCOUNT ON;
		...
		IF @ISLEM=2 -- DisOgrenci EXCEL YUKLE
			...
			If (ISJSON(@SQLJSON)=1)
				BEGIN
					...
					BEGIN
					--TC kimlik numarasý daha önce eklenmemiþ DisOgrenci tablosuna yazýlýyor
					 INSERT INTO DisOgrenci 
								(TCKIMLIKNO,AD,SOYAD,ID_KADEME3,DONEM,EPOSTA,VELI_AD_SOYAD,KYE_TCKIMLIKNO)
								SELECT TCKIMLIKNO,AD,SOYAD,ID_KADEME3,DONEM,EPOSTA,VELI_AD_SOYAD,KYE_TCKIMLIKNO 
								FROM #TMP_DisOgrenci
								WHERE TCKIMLIKNO NOT IN(SELECT TCKIMLIKNO FROM DisOgrenci)
	...
	BEGIN CATCH
		...
		SELECT @MSG = /*'Prosedür: ' + ISNULL(ERROR_PROCEDURE(), '') + ', Satýr: ' + CAST(ERROR_LINE() as varchar) + ', Hata: ' +*/ ERROR_MESSAGE()
...
--################### sp_OnlineSinav ALTER ############################


USE [Pusulam]
GO

/****** Object:  StoredProcedure [dbo].[sp_OnlineSinav]    Script Date: 30.12.2020 13:44:37 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN TRY    
		...
		BEGIN
			...
			) -- ADMÝN VEYA ÖD ÝSE ÖZEL SINAVLARI GÖRECEK			
				SET @OZELSINAVGOR=1
			
			DECLARE @QUERY NVARCHAR(MAX) = NULL

			IF @ISLEM = 1	--	Sýnav Listele
			BEGIN
				SELECT
				(
					...
					(
						SELECT 
							...
							,(
								...
								WHERE SUBSS.ID_SINAV=SOA.ID_SINAV AND SUBSS.KITAPCIK='A' AND (
															...
															OR (SUBS.ID_SORUTUR=3)						--	DOÐRU YANLIÞ
															OR (SUBS.ID_SORUTUR=4 AND SUBC.DEGER='1')	--	TEST
															OR (SUBS.ID_SORUTUR=5 AND SUBC.DEGER<>'')	--	EÞLEÞTÝRME
															OR (SUBS.ID_SORUTUR=6)						--	BOÞLUK DOLDURMA
															)
							) AS BOS,
						...
						LEFT  JOIN	SoruBankasi.dbo.Cevap				C	ON C.ID_SORU=SOR.ID_SORU AND (
																											...
																											OR (SOR.ID_SORUTUR=3 AND OC.CEVAPNO=C.CEVAPNO AND CAST(OC.CEVAP AS VARCHAR)=CAST(C.DEGER AS VARCHAR))		--	DOÐRU YANLIÞ
																											OR (SOR.ID_SORUTUR=4 AND C.DEGER='1' AND CAST(OC.CEVAP AS VARCHAR)=CAST(C.CEVAPNO AS VARCHAR))				--	TEST
																											OR (SOR.ID_SORUTUR=5 AND OC.CEVAPNO=C.CEVAPNO AND CAST(OC.CEVAP AS VARCHAR)=CAST(C.DEGER AS VARCHAR))		--	EÞLEÞTÝRME
																											OR (SOR.ID_SORUTUR=6 AND OC.CEVAPNO=C.CEVAPNO AND CAST(OC.CEVAP AS VARCHAR)=CAST(C.DEGER AS VARCHAR))		--	BOÞLUK DOLDURMA
																											)
			...
			IF @ISLEM = 7	--	Soru Listesi Getir Cevaplý
			BEGIN
				SELECT
				(
					SELECT 
			...
			IF @ISLEM = 11	--	OGRENCI ATAMA LÝSTE						
			BEGIN
			...
			IF @ISLEM = 12	--	ÖÐRENCÝ YENÝ ONLINE SINAV LÝSTESÝ		
			BEGIN
				...
				SET @QUERY = (	SELECT DEGER 
								...
								WHERE P.ID_PARAMETRE = 1) -- Öðrenci Online Sýnav Listesi
				...
				SELECT ISNULL((
					SELECT DISTINCT
						...
						,PERFORMANS	= CASE 
										...
										WHEN GETDATE() < SDO.BASTARIH							THEN 'Sýnav Henüz Baþlamadý' 
										WHEN GETDATE() > SD.ACIKLANMATARIH  AND S.DURUM != 2	THEN 'Açýklanmasý Bekleniyor' 
			...
			IF @ISLEM = 13	--	ÖÐRENCÝ YENÝ ONLINE SINAV DETAY			
			BEGIN
		...
		DECLARE @KALAN VARCHAR(MAX) =NULL;
			...
			IF @ISLEM = 14	--	ÖÐRENCÝ YENÝ ONLINE SINAV SORU ÝÞARETLE	
			BEGIN
			...
			IF @ISLEM = 15	--	ÖÐRENCÝ YENÝ ONLINE SINAV PERFORMANS	
			BEGIN
			...
			IF @ISLEM = 16	--	ÖÐRENCÝ ATAMA KALDIRMA					
			BEGIN
			...
			BEGIN
				...
				DECLARE @OGRENCI17 BIT = IIF(EXISTS(SELECT TOP 1 1 
													...
													WHERE	SY.TCKIMLIKNO		= @TCKIMLIKNO
														AND SY.ID_KULLANICITIPI = 4				--	ÖÐRENCÝ
													),1,0) 
				...
				ELSE IF NOT EXISTS(	SELECT TOP 1 1 
									FROM v3SubeYetki	SY  WITH(NOLOCK) 
									WHERE	SY.TCKIMLIKNO		= @TCKIMLIKNO
										AND SY.ID_KULLANICITIPI = 1				--	ADMÝN
									)
				BEGIN
					RETURN
					SET @CEVAPANAHTARI = 0
			...
			IF @ISLEM = 19	--	ONLINE SINAV ÖÐR OTURUM	SIFIRLAMA LÝSTE	
			BEGIN
			...
			IF @ISLEM = 20	--	ONLINE SINAV ÖÐR OTURUM	SIFIRLAMA 		
			BEGIN
