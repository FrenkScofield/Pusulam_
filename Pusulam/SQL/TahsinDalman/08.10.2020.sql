...
INSERT INTO Menu (ID_MENU,AD,ACIKLAMA,KOD,URL,RESIM,GIZLI,OZEL)
VALUES(1367, 'Öğrenci Oturum Liste', 'Öğrenci Oturum Liste', '005999007', '#OnlineSinav/OgrenciOturumListe', 'icon-list', 0, 0)
SET IDENTITY_INSERT dbo.Menu OFF;  

USE [Pusulam]
GO
/****** Object:  StoredProcedure [dbo].[sp_OnlineSinav]    Script Date: 8.10.2020 16:43:14 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN TRY    
		...
		BEGIN
			...
			BEGIN
				...
				SET @QUERY = (SELECT DEGER FROM Parametre P JOIN ParametreDeger PD ON PD.ID_PARAMETREDEGER = P.ID_PARAMETREDEGER WHERE P.ID_PARAMETRE = 1) -- Öğrenci Online Sınav Listesi
			...
			BEGIN
				...
				(
				(
					SELECT SO.TCKIMLIKNO, K.AD, K.SOYAD, SB.AD AS SUBE, S.AD AS SINIF, SOO.OTURUM, CONVERT(VARCHAR(MAX), SOO.BASTARIH, 104) + ' ' + CONVERT(VARCHAR(MAX), SOO.BASTARIH, 108) AS TARIH
					FROM Sinav SNV 
					JOIN Tbl_OnlineSinavOgrenci SO ON SO.ID_SINAV = SNV.ID_SINAV AND SO.AKTIF = 1
					JOIN Tbl_OnlineSinavOgrenciOturum SOO ON SOO.TCKIMLIKNO = SO.TCKIMLIKNO AND SOO.ID_SINAV = SO.ID_SINAV
					JOIN OkyanusDB.dbo.v3Kullanici K ON K.TCKIMLIKNO = SOO.TCKIMLIKNO
					JOIN OkyanusDB.dbo.v3OgrenciTum O ON O.TCKIMLIKNO = K.TCKIMLIKNO AND O.DONEM = SNV.DONEM
					JOIN OkyanusDB.dbo.v3Sube SB ON SB.ID_SUBE = O.ID_SUBE
					JOIN OkyanusDB.dbo.v3SinifTum S ON S.ID_SINIF = O.ID_SINIF AND S.DONEM = SNV.DONEM
					WHERE SNV.AKTIF = 1 AND SNV.ID_SINAV = @ID_SINAV
					AND (O.ID_SUBE	IN (select value from openjson (@ID_SUBELIST))	OR	(0 IN (select value from openjson (@ID_SUBELIST))) OR @ID_SUBELIST='[]')
