...
GO
PRINT N'Altering [dbo].[Tbl_OnlineSinavDetayOturum]...';


GO
ALTER TABLE [dbo].[Tbl_OnlineSinavDetayOturum]
    ADD [MAIL] BIT CONSTRAINT [DF_Tbl_OnlineSinavDetayOturum_MAIL] DEFAULT ((0)) NOT NULL;


GO
PRINT N'Creating [dbo].[Tbl_OnlineSinavOgrenciOturum].[IX_Tbl_OnlineSinavOgrenciOturum_TCKIMLIKNO_ID_SINAV_OTURUM]...';


GO
CREATE NONCLUSTERED INDEX [IX_Tbl_OnlineSinavOgrenciOturum_TCKIMLIKNO_ID_SINAV_OTURUM]
    ON [dbo].[Tbl_OnlineSinavOgrenciOturum]([TCKIMLIKNO] ASC, [ID_SINAV] ASC, [OTURUM] ASC);
...
END
GO
PRINT N'Creating [dbo].[sp_OnlineSinavMailYolla]...';


GO

CREATE PROCEDURE sp_OnlineSinavMailYolla
AS
BEGIN

-- KADİR CENGİZ 11.11.2020
-- ONLINE SINAV MAIL YOLLAMA


	DROP TABLE IF EXISTS #WHILE
	SELECT DISTINCT D.ID_SINAV,O.OTURUM, RN = O.ID_ONLINESINAVDETAYOTURUM--ROW_NUMBER() OVER(ORDER BY D.ID_SINAV, O.OTURUM)
	INTO #WHILE
	FROM Tbl_OnlineSinavDetay		D 
	JOIN Tbl_OnlineSinavDetayOturum O ON O.ID_ONLINESINAVDETAY = D.ID_ONLINESINAVDETAY
	WHERE	D.AKTIF		= 1
		AND O.BITTARIH	< GETDATE()
		AND O.MAIL		= 0


	WHILE EXISTS(SELECT TOP 1 1 FROM #WHILE )
	BEGIN
		DECLARE @ID_SINAV		INT
		DECLARE @SINAVOTURUM	INT
		DECLARE @RN				INT = (SELECT TOP 1 RN FROM #WHILE ORDER BY RN)
		DECLARE @TCKIMLIKNO		VARCHAR(MAX) = '22723353928' -- METE DOĞANAY
		DECLARE @OTURUM			VARCHAR(MAX) = (SELECT OTURUM FROM v3Kullanici WHERE TCKIMLIKNO = @TCKIMLIKNO)

		DECLARE @TABLE TABLE ( RESULT VARCHAR(MAX), RN INT)
		SELECT TOP 1 
				@ID_SINAV		= ID_SINAV
			,@SINAVOTURUM	= OTURUM
		FROM #WHILE 
		...
		INSERT INTO @TABLE
		EXEC sp_Sinav @ISLEM = 48, @ID_SINAV = @ID_SINAV, @SINAVOTURUM = @SINAVOTURUM, @TCKIMLIKNO = @TCKIMLIKNO, @OTURUM = @OTURUM, @MAIL = 1
		...
		BEGIN
			SET @TBODY += (SELECT TOP 1 RESULT FROM @TABLE ORDER BY RN) + '<BR>'
			DELETE FROM @TABLE WHERE RN = (SELECT TOP 1 RN FROM @TABLE ORDER BY RN)
		END

		DECLARE @KONU VARCHAR(MAX) = CAST(@ID_SINAV as varchar) + ' Id li Sınav ' + CAST(@SINAVOTURUM as varchar) + '. Oturum Optik Listesi'

		DECLARE @MAIL VARCHAR(MAX)='metedoganay@okyanuskoleji.k12.tr'	
		...
		UPDATE Tbl_OnlineSinavDetayOturum SET 
			MAIL = 1
		WHERE ID_ONLINESINAVDETAYOTURUM = @RN

		DELETE FROM #WHILE WHERE RN = @RN
	END
END
GO
