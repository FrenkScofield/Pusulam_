...
ALTER TABLE Tbl_OnlineSinavOgrenciCevap  ADD KYI_TARIH DATETIME
go
USE [Pusulam]
GO
/****** Object:  StoredProcedure [dbo].[sp_OnlineSinav]    Script Date: 31.03.2021 13:40:18 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN TRY    
		...
		BEGIN
				...
				BEGIN
					...
					JOIN v3Kullanici AS K ON K.TCKIMLIKNO=OSO.KYE_TCKIMLIKNO
						WHERE OSO.ID_SINAV=@ID_SINAV
							 ...
							 -- AND OSO.KYE_TCKIMLIKNO='14531453000'
							 AND OSO.AKTIF=1
			...
			BEGIN
				...
				WHERE	OS.ID_SINAV		= @ID_SINAV 
					AND OS.TCKIMLIKNO	= @TC_OGRENCI
				...
				SELECT ISNULL((
					SELECT DISTINCT 
						...
						SORUNO,
						CEVAP,
						CONVERT(VARCHAR(10), KYE_TARIH, 103) + ' '  + convert(VARCHAR(8), KYE_TARIH, 14) AS KYE_TARIH,
						CONVERT(VARCHAR(10), KYI_TARIH, 103) + ' '  + convert(VARCHAR(8), KYI_TARIH, 14) AS KYI_TARIH
					FROM #SinavLogTempTable 
					ORDER BY KYE_TARIH
					FOR JSON PATH
				),'[]')
			...
			IF @ISLEM = 31	--	SINAVA GİREN DIŞ ÖĞRENCİ LİSTESİ		
			BEGIN
				SELECT ISNULL((
					SELECT DISTINCT  DO.TCKIMLIKNO,DO.AD+' '+ DO.SOYAD  AS ADSOYAD
					FROM Tbl_OnlineSinavOgrenci OSO
					JOIN DisOgrenci				DO	ON	DO.TCKIMLIKNO	= OSO.TCKIMLIKNO
					WHERE	OSO.AKTIF	= 1 
						AND OSO.ID_SINAV= @ID_SINAV  
					FOR JSON AUTO
				),'[]')
