USE [Pusulam]
GO
/****** Object:  StoredProcedure [dbo].[sp_SportifKulup]    Script Date: 11.08.2021 17:43:28 ******/
SET ANSI_NULLS ON
GO
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					WHERE KK.ID_SUBE = (SELECT TOP 1 ID_SUBE FROM v3Ogrenci WITH (NOLOCK) WHERE TCKIMLIKNO = @TCKIMLIKNO) 
						AND KK.Kota > (SELECT COUNT(1) 
			...
			BEGIN				
				 SELECT ISNULL((
					SELECT * FROM KulupKota WHERE ID_SUBE = @ID_SUBE AND ID_KULUP = @ID_KULUP
					FOR JSON PATH
				),'[]')
			END
