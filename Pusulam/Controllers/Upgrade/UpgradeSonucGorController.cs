...
{
    [GzipCompression]
    public class UpgradeSonucGorController : ApiController
    {
        internal int ID_MENU = (int)EMenu.UpgradeSonucGor;
        ...
        public Object UpgradeOgrenciSonucListele(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DUpgradeSoru.ID_MENU = ID_MENU;
                    return c.DUpgradeSoru.UpgradeOgrenciSonucListele(j);
                }
            }
            catch (Exception ex)
            {
