...
{
    [GzipCompression]
    public class OI_OBSListesiController : ApiController
    {

        internal int ID_MENU = (int)EMenu.OBSListesi;
        ...
        public Object OBSListele(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DOgrenci.ID_MENU = ID_MENU;
                    return c.DOgrenci.OBSListele(j);
                }
            }
            catch (Exception)
            {
