...
{
    [GzipCompression]
    public class DegerlendirmeOgrenciController : ApiController
    {
        internal int ID_MENU = (int)EMenu.DegerlendirmeOgrenci;
        ...
        public Object OgrenciListeleV4(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DDegerlendirme.ID_MENU = ID_MENU;
                    return c.DDegerlendirme.OgrenciListeleV4(j);
                }
            }
            catch (Exception)
            {
