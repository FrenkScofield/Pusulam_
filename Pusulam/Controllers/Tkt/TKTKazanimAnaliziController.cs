...
{
    [GzipCompression]
    public class TKTKazanimAnaliziController : ApiController
    {
        internal int ID_MENU = (int)EMenu.TKTKazanimAnalizi;
        ...
        public Object SinifListelebyKullaniciDonemMultiTekGrup(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DSinif.ID_MENU = ID_MENU;
                    return c.DSinif.SinifListelebyKullaniciDonemMultiTekGrup(j);
                }
            }
            catch (Exception)
            {
