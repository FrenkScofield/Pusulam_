...
{
    [GzipCompression]
    public class RehberlikEnvanterTanimlamaController : ApiController
    {
        ...
        public Object RehberlikEnvanterListele(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DRehberlikEnvanter.ID_MENU = ID_MENU;
                    return c.DRehberlikEnvanter.RehberlikEnvanterListele(j);
                }
            }
            catch (Exception)
            {
        ...
        public Object RehberlikEnvanterAktifPasif(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DRehberlikEnvanter.ID_MENU = ID_MENU;
                    return c.DRehberlikEnvanter.RehberlikEnvanterAktifPasif(j);
                }
            }
            catch (Exception)
            {
