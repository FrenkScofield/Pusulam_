...
{
    [GzipCompression]
    public class FrekansSinifV2Controller : ApiController
    {

        internal int ID_MENU = (int)EMenu.FrekansSinifV2;
        ...
        public Object FrekansSinifSonFiltre(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DFrekansV2.ID_MENU = ID_MENU;
                    return c.DFrekansV2.FrekansSinifSonFiltre(j);
                }
            }
            catch (Exception)
            {
