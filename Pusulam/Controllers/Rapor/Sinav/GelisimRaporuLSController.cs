...
{
    [GzipCompression]
    public class GelisimRaporuLSController : ApiController
    {
        internal int ID_MENU = (int)EMenu.GelisimRaporuLS;
        ...
        public Object GelisimRaporuHTML(JObject j)
        {
            try
            {
                using (Channel c = new Channel())
                {
                    c.DSinav.ID_MENU = ID_MENU;
                    return c.DSinav.GelisimRaporLSHTML(j);
                }
            }
            catch (Exception ex)
            {
