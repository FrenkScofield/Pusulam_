...
{
    public class DKotaBelirle : DBase
    {
        ...
        public List<MSinavGrup> Kademe3ListelebyKullanici(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_SportifKulup.Kademe3ListelebyKullanici);
                ...
                using (IDbConnection db = new SqlConnection(conStr))
                {
                    ...
                    liste = db.Query<MSinavGrup>("sp_SportifKulup", j.ToDictionary(), commandTimeout: 600, commandType: CommandType.StoredProcedure).ToList();
                }
                return liste;
            }
            catch (Exception ex)
