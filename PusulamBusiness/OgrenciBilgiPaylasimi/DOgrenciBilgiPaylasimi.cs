...
namespace PusulamBusiness.OgrenciBilgiPaylasimi
{
    public class DOgrenciBilgiPaylasimi : DBase

    {
        GetIp getIp = new GetIp();
        public string BilgiEkle(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_OgrenciBilgiSistemi.BilgiEkle);
        ...
        public string BilgiListele(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_OgrenciBilgiSistemi.BilgiListele);
                j.Add("ID_MENU", ID_MENU);
                j.Add("IP", getIp.GetUser_IP());
        ...
        public string BilgiSil(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_OgrenciBilgiSistemi.BilgiSil);
                j.Add("ID_MENU", ID_MENU);
                j.Add("IP", getIp.GetUser_IP());
        ...
        public string BilgiGetir(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_OgrenciBilgiSistemi.BilgiGetir);
                j.Add("ID_MENU", ID_MENU);
                j.Add("IP", getIp.GetUser_IP());
        ...
        public string BilgiDuzenle(JObject j)
        {
            try
            {
                j.Add("ISLEM", (int)sp_OgrenciBilgiSistemi.BilgiDuzenle);
                j.Add("ID_MENU", ID_MENU);
                j.Add("IP", getIp.GetUser_IP());
        ...
        public bool DosyaKaydet()
        {
            try
            {
                ...
                string ID_OGRENCIBILGI = (HttpContext.Current.Request.Form["ID_OGRENCIBILGI"] != null) ? HttpContext.Current.Request.Form["ID_OGRENCIBILGI"].ToString() : "";
                ...
                for (int i = 0; i < HttpContext.Current.Request.Files.Count; i++)
                {
                    ...
                    if (DOSYAGUID == "" && file != null)
                    {
                        if (file != null)
                        {
                            ...
                            sonuc = AmazonDosyaYukle.sendMyFileToS3(@"pusulam/OgrenciBilgiPaylasimi/Bilgiler", GUID.ToString(), fileToUpload, file.ContentType, TCKIMLIKNO);
                        }
                    }
                }
                ...
                j.Add("ID_MENU", ID_MENU);
                j.Add("ID_OGRENCIBILGI", ID_OGRENCIBILGI);
                ...
                using (IDbConnection db = new SqlConnection(conStr))
                {
                    if (db.State == ConnectionState.Closed)
                        db.Open();
                    int result = db.Execute("sp_OgrenciBilgiSistemi", j.ToDictionary(), commandTimeout: 600, commandType: CommandType.StoredProcedure);
                    return result > 0;
                }
            }
            catch (Exception ex)
