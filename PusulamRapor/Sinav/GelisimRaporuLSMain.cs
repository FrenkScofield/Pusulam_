...
{
    public partial class GelisimRaporuLSMain : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public GelisimRaporuLSMain(string tckimlikno, string oturum, string tcogrenci, string idsinif, string raporturlist, string donem, string tur, string idmenu)
        {
            InitializeComponent();
        ...
        private void GelisimRaporuLSMain_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                using (Baglanti b = new Baglanti())
                {
                    ...
                    ds = b.SorguGetir("sp_GelisimRaporuLS");
        ...
        private void GroupHeader1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            GelisimRaporuLS rapor = new GelisimRaporuLS(ds, TC, DONEM, RAPORTURLIST,OTURUM);
            xrGelisimRaporuLS.ReportSource = rapor;
        ...
        private void GelisimRaporuLSMain_AfterPrint(object sender, EventArgs e)
        {
            if (TUR)
            {
                string[] sourcefiles = new string[os];
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/Temp/" + OTURUM + ""));
                int tss = 0;
                for (int i = 0; i < os; i++)
                {
                    ...
                    string name = names[i];
                    string path = HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/Temp/" + OTURUM + "/" + name + ".pdf");
                    sourcefiles[i] = path;
                    newReport.ExportToPdf(path);
                }

                if (File.Exists(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/" + OTURUM + ".zip")))
                {
                    File.Delete(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/" + OTURUM + ".zip"));
                }
                ...
                using (ZipArchive archive = new ZipArchive())
                {
                    ...
                    archive.Save(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/" + OTURUM + ".zip"));
                }

                System.IO.DirectoryInfo di = new DirectoryInfo(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/Temp/" + OTURUM + ""));
                ...
                Directory.Delete(HttpContext.Current.Server.MapPath("/Dosyalar/GelisimRaporuLS/Temp/" + OTURUM + ""), true);
            }
        }
    }
}
